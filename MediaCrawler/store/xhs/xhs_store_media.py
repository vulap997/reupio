...
def _xhs_leaf():
    """ViralCrawl: nền tảng app 'rednote' (bản quốc tế) tách data/rednote khỏi data/xhs (nội địa).
    Đặt env MC_XHS_LEAF=rednote cho crawl rednote; mặc định 'xhs' (giữ hành vi cũ)."""
    return os.environ.get("MC_XHS_LEAF", "xhs")
...
class XiaoHongShuImage(AbstractStoreImage):
    def __init__(self):
        ...
        if config.SAVE_DATA_PATH:
            self.image_store_path = f"{config.SAVE_DATA_PATH}/{leaf}/images"
        else:
            self.image_store_path = f"data/{leaf}/images"
...
class XiaoHongShuVideo(AbstractStoreVideo):
    def __init__(self):
        leaf = _xhs_leaf()
        if config.SAVE_DATA_PATH:
            self.video_store_path = f"{config.SAVE_DATA_PATH}/{leaf}/videos"
        else:
            self.video_store_path = f"data/{leaf}/videos"
    ...
    async def store_video(self, video_content_item: Dict):
        ...
        """
        await self.save_video(video_content_item.get("notice_id"), video_content_item.get("video_content"), video_content_item.get("extension_file_name"), video_content_item.get("sub_dir", ""))

    def make_save_file_name(self, notice_id: str, extension_file_name: str, sub_dir: str = "") -> str:
        """
        make save file name by store type
        ...
        Args:
            ...
            sub_dir: thư mục con tùy chọn (tu-khoa/<kw>, kenh/<tên>, link) — chia thư mục như Douyin

        Returns:

        """
        if sub_dir:
            return f"{self.video_store_path}/{sub_dir}/{extension_file_name}"
        return f"{self.video_store_path}/{notice_id}/{extension_file_name}"

    async def save_video(self, notice_id: str, video_content: str, extension_file_name, sub_dir: str = ""):
        """
        save video to local
        ...
        """
        save_file_name = self.make_save_file_name(notice_id, extension_file_name, sub_dir)
        pathlib.Path(save_file_name).parent.mkdir(parents=True, exist_ok=True)
        async with aiofiles.open(save_file_name, 'wb') as f:
