...
def _xhs_media_parts(note_item):
    """Trả (sub_dir, file_base) — CHIA thư mục lưu video XHS theo từ-khoá/kênh GIỐNG Douyin:
    search -> tu-khoa/<kw>; creator -> kenh/<nickname>; detail -> link.
    file_base = '<title>_<6 ký tự cuối note_id>' (độc nhất, nhiều video cùng từ khoá KHÔNG đè nhau)."""
    note = note_item if isinstance(note_item, dict) else {}
    nid = note.get("note_id", "")
    # Tên file = tiêu đề ĐÃ DỊCH sang ngôn ngữ đích (không chỉ dịch trên màn hình)
    title = _xhs_safe_name(dich_ten.dich_tieu_de(note.get("title") or note.get("desc") or "", "xhs"), 50)
    file_base = f"{title}_{str(nid)[-6:]}" if nid else (title or "video")
    ctype = config.CRAWLER_TYPE
    if ctype == "search":
        sub_dir = f"tu-khoa/{_xhs_safe_name(source_keyword_var.get() or 'khac', 40)}"
    elif ctype == "creator":
        nick = (note.get("user", {}) or {}).get("nickname", "") or note.get("nickname", "") or "khong-ro"
        sub_dir = f"kenh/{_xhs_safe_name(nick, 40)}"
    else:
        sub_dir = "link"
...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    def __init__(self) -> None:
        ...
        # ViralCrawl: PHẢI khớp UA lúc login (mo_dang_nhap = Windows Chrome 126). XHS gắn phiên theo
        # fingerprint UA — cookie sinh dưới Windows mà cào gửi Mac → bất nhất → kích anti-bot/CAPTCHA/-104.
        ...
        async with async_playwright() as playwright:
            ...
            if not await self.xhs_client.pong():
                if os.environ.get("MC_NO_QR_LOGIN", "1") != "0":   # app crawl: KHÔNG tự bật QR popup (tmp*.PNG) khi chưa login -> fail gọn; login qua nút Đăng nhập
                    utils.logger.error("[XiaoHongShuCrawler] Chưa đăng nhập Xiaohongshu — KHÔNG cào được. Hãy đăng nhập qua nút Đăng nhập trong app.")
                    return
            ...
            while (page - start_page + 1) * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    if not notes_res or not notes_res.get("has_more", False):
                        ...
                        break
                    # Xem trước (preview, MC_GET_MEDIAS=0): tăng song song lấy thumbnail/chi tiết cho NHANH
                    # (đã login = duyệt bình thường). Tải thật giữ MAX_CONCURRENCY_NUM cho an toàn chống-bot.
                    _conc = config.MAX_CONCURRENCY_NUM if config.ENABLE_GET_MEIDAS else max(config.MAX_CONCURRENCY_NUM, 4)
    ...
    async def get_note_detail_async_task(
        ...
        async with semaphore:
            try:
                ...
                # Sleep chống-bot SAU mỗi detail — CHỈ khi TẢI THẬT. Xem trước (preview) bỏ sleep 2s/bài
                # (nằm trong semaphore → giữ slot → 20 bài nối tiếp ~40s); chỉ jitter nhỏ tránh burst đồng loạt.
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            ...
            _prof = os.environ.get("MC_XHS_PROFILE") or (config.USER_DATA_DIR % config.PLATFORM)  # ViralCrawl: rednote dùng profile RIÊNG (rednote_user_data_dir) tách khỏi xhs nội địa
            user_data_dir = os.path.join(os.environ.get("MC_BROWSER_DATA_DIR") or os.path.join(os.getcwd(), "browser_data"), _prof)  # type: ignore
    ...
    async def get_notice_video(self, note_item: Dict):
        ...
        if not videos:
            return
        videoNum = 0
        sub_dir, file_base = _xhs_media_parts(note_item)
        for url in videos:
            content = await self.xhs_client.get_note_media(url)
            await asyncio.sleep(random.random())
            if content is None:
                continue
            extension_file_name = f"{file_base}.mp4" if videoNum == 0 else f"{file_base}_{videoNum}.mp4"
            videoNum += 1
            await xhs_store.update_xhs_note_video(note_id, content, extension_file_name, sub_dir=sub_dir)
