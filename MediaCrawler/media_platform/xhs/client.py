...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    def __init__(
        self,
        timeout=config.XHS_API_TIMEOUT,  # ViralCrawl: env MC_XHS_TIMEOUT (mặc định 60). Hạ để cắt treo do anti-bot.
        proxy=None,
        *,
    ...
    async def pong(self) -> bool:
        ...
        try:
            ...
            if self_info and self_info.get("data", {}).get("result", {}).get("success"):
                ping_flag = True
            elif self_info:   # ViralCrawl: lộ lý do (code/msg) để UI phân biệt -104 "không có quyền" vs chưa-login
                utils.logger.warning(
                    f"[XiaoHongShuClient.pong] not logged in: code={self_info.get('code')} msg={self_info.get('msg')}")
