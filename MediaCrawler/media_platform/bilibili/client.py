...
class BilibiliClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def get_wbi_keys(self) -> Tuple[str, str]:
        ...
        if wbi_img_urls and "-" in wbi_img_urls:
            ...
            resp = await self.request(method="GET", url=self._host + "/x/web-interface/nav", headers=self.headers)
    ...
    async def get_video_media(self, url: str) -> Union[bytes, None]:
        ...
        # - Tải NGUYÊN file 1 request (Range bytes=0-) -> CDN ĐÓNG kết nối với video LỚN (RemoteProtocolError;
        #   56MB OK nhưng 274MB rớt). FIX: tải theo KHỐI Range ~8MB (như browser stream HLS) + retry MỖI khối.
        base_headers = {
        ...
        while True:
            ...
            for attempt in range(3):
                try:
                    # CLIENT MỚI MỖI KHỐI: tái dùng 1 httpx client cho NHIỀU request tới CDN akamaized bị TREO
                    # (giữ keep-alive nhưng CDN không phản hồi request sau) -> phải tạo client mới mỗi khối.
                    async with make_async_client(proxy=self.proxy, follow_redirects=True) as client:
                        response = await client.request("GET", url, timeout=media_timeout, headers=headers)
                        status = response.status_code
                        content = response.content if status in (200, 206) else b""
                        if total_size is None and status == 206:
                            cr = response.headers.get("content-range", "")   # "bytes 0-X/TỔNG"
                            if "/" in cr:
                                try:
                                    total_size = int(cr.rsplit("/", 1)[1])
                                except Exception:
                                    pass
                    if status == 416:   # Range vượt EOF -> hết file
                        data = b""
                        break
                    if status in (200, 206):
                        data = content
                        break
                    utils.logger.error(f"[BilibiliClient.get_video_media] status {status} for {url}")
                    return None
                except httpx.HTTPError as exc:   # RemoteProtocolError/ReadTimeout... -> thử lại khối này
                    last_exc = exc
                    utils.logger.warning(f"[BilibiliClient.get_video_media] {exc.__class__.__name__} khối @{start//1048576}MB (lần {attempt + 1}/3)")
            if data is None:
                utils.logger.error(f"[BilibiliClient.get_video_media] Thất bại khối @{start//1048576}MB sau 3 lần ({last_exc.__class__.__name__ if last_exc else '?'})")
                return None
            if not data:
                break
            parts.append(data)
            start += len(data)
            if total_size and len(parts) % 4 == 0:   # mỗi ~32MB -> log % cho UI (Nhật ký) biết ĐANG tải
                utils.logger.info(f"📥 Đang tải video: {round(start/1048576)}/{round(total_size/1048576)}MB ({round(start/total_size*100)}%)")
            if len(data) < CHUNK:   # server trả ít hơn yêu cầu -> đã là khối CUỐI
                break
        if not parts:
            return None
        utils.logger.info(f"[BilibiliClient.get_video_media] tải xong {round(start/1048576,1)}MB theo {len(parts)} khối")
        return b"".join(parts)
